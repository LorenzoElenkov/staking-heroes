<template>
  <div v-if="!wallet" class="text-center text-3xl text-white mb-10">Choose your wallet provider</div>
  <div v-if="!wallet"><ConfigPane /></div>
  <div v-if="wallet">
    <!--farm address-->
    <div v-if="farmerAcc">
      <FarmerDisplay
      ...
      <Vault
        :key="farmerAcc"
        class="mb-10 flex desktop:flex-row mobile:flex-col w-full"
        :vault="farmerAcc.vault.toBase58()"
        @selected-wallet-nft="handleNewSelectedNFT"
      >
        <button
          v-if="farmerState === 'staked' && selectedNFTs.length > 0"
          class="nes-btn is-primary mr-5 mobile:flex-1"
          @click="addGems"
        >
          Add Heroes (resets staking)
        </button>
        <button
          v-if="farmerState === 'unstaked'"
          class="nes-btn is-success mr-5 mobile:flex-1"
          @click="beginStaking"
        >
          Begin staking
        </button>
        <button
          ...
          class="nes-btn is-error mr-5 mobile:flex-1"
          @click="endStaking"
        >
          End staking
        </button>
        ...
        <button class="nes-btn is-warning mobile:flex-1" @click="claim">
          Claim <span class="text-2xl text-red-800">{{ availableA }}</span> HROM
        </button>
      </Vault>
    </div>
  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    // --------------------------------------- farmer details
    const farm = ref<any>('ChYBE8XVMN1kcPGhkih6R5if9T3ypejoYfPJMsUd3Hop');
